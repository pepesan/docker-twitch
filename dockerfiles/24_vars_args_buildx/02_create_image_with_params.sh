...
docker build --build-arg APP_MODE=prod -t $DOCKER_HUB_USER/$DOCKER_HUB_REPOSITORY:latest .
## define otro tag con la fecha actual
docker build --build-arg APP_MODE=prod -t $DOCKER_HUB_USER/$DOCKER_HUB_REPOSITORY:20251204 .
