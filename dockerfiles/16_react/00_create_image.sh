...
docker build -t $DOCKER_HUB_USER/$REPO_NAME:latest .
## define otro tag con la fecha actual
docker build -t $DOCKER_HUB_USER/$REPO_NAME:20251111 .
## define otro tag con la versión de nuestro software
docker build -t $DOCKER_HUB_USER/$REPO_NAME:1.0.0 .
